...
import { Map, User, Car, Shield } from "lucide-react";
...
export default function MainLayout() {
  ...
  const isAdmin = useMemo(() => {
    const me = getStoredUser();
    return me?.roleName?.toUpperCase() === "ADMIN";
  }, []);
    ...
    <div className="flex flex-col h-screen bg-[#FFFDF5] text-black font-bold overflow-hidden">
      ...
      {isAdmin && (
        <div className="fixed top-4 right-4 z-50">
          <Link
            to="/admin"
            className="flex items-center gap-2 px-4 py-2 bg-[#00D1FF] text-black font-black rounded-lg border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:bg-[#00B8E6] transition-colors"
          >
            <Shield size={20} />
            <span>Admin</span>
          </Link>
